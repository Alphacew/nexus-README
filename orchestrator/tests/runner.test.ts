...
import { runParserBinary, BinaryRunnerError, resolveBinaryPath } from '../src/runner.js';
...
test('Zod Schema validation', () => {
  ...
  assert.strictEqual(invalidResult.success, false, 'Schema should reject partial payloads');
});
...
test('Parser binary integration test', async () => {
  ...
  try {
    ...
    assert.strictEqual(topology.projectName, 'nexus-readme', 'Project name should match folder');
    ...
    const mainModule = topology.modules.find((m) => m.filePath.endsWith('src/main.rs'));
