...
test("CLI: execution flow creates file and performs atomic swap", async () => {
  ...
  try {
    await main([
      ...
      dummyOutput,
      "-e",
      "orchestrator/node_modules,orchestrator/dist",
    ]);
    ...
    assert.ok(
      content.includes("<!-- NEXUS_START:OVERVIEW -->"),
      "Overview should exist",
    );
    assert.ok(
      content.includes("<!-- NEXUS_START:GRAPH -->"),
      "Graph should exist",
    );
    assert.ok(
      content.includes("<!-- NEXUS_START:ARCHITECTURE -->"),
      "Architecture should exist",
    );
    assert.ok(
      content.includes("<!-- NEXUS_START:REFERENCE -->"),
