...
test("Agent Pipeline: falls back to stubs if no API key is provided and useMock is not explicitly set", async () => {
  ...
  try {
    ...
    assert.ok(
      result.OVERVIEW.includes("test-project"),
      "Overview should mention project name",
    );
    assert.ok(
      result.ARCHITECTURE.includes("cargo build") ||
        result.ARCHITECTURE.includes("npm run build"),
      "Architecture quickstart instructions should exist",
    );
    assert.ok(
      result.REFERENCE.includes("src/main.ts"),
      "Reference table should contain the module files",
    );
  } finally {
...
test("Agent Pipeline: respects mockResponses override configuration", async () => {
  ...
  assert.strictEqual(
    result.ARCHITECTURE,
    "Custom Quickstart Setup Instructions",
  );
